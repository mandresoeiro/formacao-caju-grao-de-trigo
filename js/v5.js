(() => {
  ...
  function initStudentSignupTools() {
    const forms = Array.from(document.querySelectorAll("[data-student-form]"));
    if (!forms.length) return;

    forms.forEach(form => {
      const status = form.querySelector("[data-student-status]") || document.querySelector("[data-student-status]");
      ...
      form.addEventListener("submit", async event => {
        ...
        const data = {
          ...
          interest: String(fd.get("interest") || "Participar da formação").trim(),
